...
class Trainer(object):
    ...
    def test(model, args, data_loader, scaler, logger, path=None):
        ...
        for t in range(y_true.shape[1]):
            mae, rmse, mape, r2, _, _ = All_Metrics(y_pred[:, t, ...], y_true[:, t, ...],
                                                 args.mae_thresh, args.mape_thresh)
            logger.info("Horizon {:02d}, MAE: {:.2f}, RMSE: {:.2f}, MAPE: {:.4f}%, R2: {:.4f}".format(
                t + 1, mae, rmse, mape*100, r2))
        mae, rmse, mape, r2, _, _ = All_Metrics(y_pred, y_true, args.mae_thresh, args.mape_thresh)
        logger.info("Average Horizon, MAE: {:.2f}, RMSE: {:.2f}, MAPE: {:.4f}%, R2: {:.4f}".format(
                    mae, rmse, mape*100, r2))
        
        # Report parameter count during evaluation
        total_params = sum(p.numel() for p in model.parameters())
        logger.info("Model Parameters: {}".format(total_params))

        # CSV results export
        import csv
        csv_path = r"c:\Users\student\paperTest\results_agcrn.csv"
        ...
        if rows:
            ...
            for r in rows[1:]:
                if r and r[0] != args.dataset:
                    new_rows.append(r)
        else:
            new_rows.append(["Dataset", "MAE", "RMSE", "MAPE", "R2", "Parameters"])
        
        new_rows.append([args.dataset, "{:.4f}".format(mae), "{:.4f}".format(rmse), "{:.4f}".format(mape), "{:.4f}".format(r2), str(total_params)])
        
        try:
            with open(csv_path, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerows(new_rows)
            logger.info("Results exported successfully to {}".format(csv_path))
        except Exception as e:
            logger.error("Error exporting results to CSV: {}".format(e))
