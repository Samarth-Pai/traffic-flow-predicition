...
def split_data_chronological(data, train_ratio, val_ratio, test_ratio):
...
def get_dataloader(args, normalizer = 'std', tod=False, dow=False, weather=False, single=True):
    #load raw st dataset
    dataset_path = getattr(args, 'dataset_path', args.dataset)
    data = load_st_dataset(dataset_path)        # B, N, D
    #normalize st data
    data, scaler = normalize_dataset(data, normalizer, args.column_wise)
    #spilit dataset chronologically
    train_ratio = getattr(args, 'train_ratio', 0.7)
    val_ratio = args.val_ratio
    test_ratio = args.test_ratio
    data_train, data_val, data_test = split_data_chronological(data, train_ratio, val_ratio, test_ratio)
    #add time window
